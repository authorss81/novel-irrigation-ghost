...
5. `outline/volume-01.md` — **especially the Midpoint reversal and the Movement III section.** This batch is the midpoint of the volume and it contains the reveal the whole first half has been walking towards.
6. `outline/batches/volume-01-batch-0001.md` — the *Aftermath and handoff state* section.
7. `state/current.md`, `state/continuity.md`, `state/open-threads.md`, `state/character-state.md`
8. `state/batch-0001-summary.md`, `state/batch-0002-summary.md`, and the **Chapter 0011–0020 entries** in `state/chapter-summaries.md`
9. `bible/premise.md`, `bible/world.md`, `bible/power-system.md`, `bible/characters.md`, `bible/terminology.md`, `bible/themes.md`
10. **The previous ten chapters, in full: `chapters/volume-01/chapter-0011.md` through `chapter-0020.md`.** Then, if the context budget allows, read `chapter-0003.md`, `chapter-0005.md`, `chapter-0006.md` and `chapter-0010.md` again, because the descent in this batch pays off Chapter 3's blue doors, Chapter 5's cracked Ashgate stone, Chapter 6's named branch and Chapter 10's eleven hours.
...
Write **Chapters 21–30** as complete finished scenes. This is the volume's physical turn: the descent, the chamber, the crowd, and the cost. The batch has to deliver three things the whole volume has been promising and one thing it has been avoiding.

1. **Ashgate is not an empty ruin. It is intact, and it sits beneath a newer settlement.** This is the midpoint reversal, and it is deliberately *not a conspiracy*. After the official closure, people were housed on top of the sealed works. The district of occupied houses Tarin found on Day 1 has been standing on a pressurised void that nobody who lives there has been told about. The mystery stops being "is the ledger lying" and becomes "who is living on top of the truth, and who put them there."
2. **The ghost is a crowd, not a woman.** In Chapter 24 Tarin hears more than one voice where he expected his mother's, and none of them will tell him anything useful. Establish it once, plainly, and do not explain it. It is the accumulated pressure of unlisted routes and their people.
3. **The cost.** The descent damages somebody's ability to work, the return route is unstable, the settlement above learns what it is standing on, and Halden's falsified measurements are placed in context: he is concealing illegal contractor pumping through a reservoir his own ledger calls dead. He is the volume's villain and he is no more than that. He is not removed here.

## Fixed starting position — Day 34 evening
...
The calendar is elastic inside roughly Days 34–50. Push dates rather than compress scenes.
...
| 26 | 40–41 | The cost chapter. The lane learns what it is standing on. Nobody is allowed to be a symbol about it. |
| 27 | 42 | Reading day in Vellum, if the batch has earned it — or its consequence. |
| 28 | 43–44 | Pressure: the district has to decide what to do with what it now knows, and it cannot be allowed to stay undecided. |
| 29 | 45 | Recovery or escalation, alternating. Do not run two action chapters together. |
| 30 | 46 | Close the movement on a completed beat that hands Movement IV a live problem. |

**The midsummer review is not forced until Chapter 46.** It may be quoted, deferred, or argued about. Movement IV is crosscurrent, consent and Tarin's old closure; Movement V is the review.
...
- **The gauge log reads `5·89` in Office notation and the engineer says "five point eight nine."** Keep that split. Record keepers write it; the engineer says it.
- **A channel stops at the balance point.** Any attempt reasoned in this batch is reasoned from the same physics.
- **A gauged stop-log meters a branch; an ungauged one does not.**
- **The eleven-hour window does not repeat and no third window is won in this movement.** The number stays eleven because the source does not grow.
- **Two charges remain open, never merged, upgraded or resolved.** A hearing may be mentioned. Do not hold it.
...
## The collective voice — Chapter 24, once

**Chapters 1–23 must not confirm it, must not name it, and must not let a character say it aloud.** Chapter 24 is where the many-voiced reading happens, and it is inside Volume 1, so this batch is the last place it can be spent before the reader has it. It is established once, plainly, and **not explained**. None of the voices is Lira. None of them is helpful. None of them tells him anything he can use. The crowd is not a villain and it is not asking for anything; it is pressure with a history in it. Do not re-use the Chapter 5 or Chapter 10 pressure images, and do not re-use a crowd image for the *second* sound in earlier chapters — those were a room and a rope, and they stay that way.
...
Update `state/batch-0003-summary.md`, `state/chapter-summaries.md`, `state/continuity.md`, `state/open-threads.md`, `state/current.md`, and `state/character-state.md`.

Then read `outline/volume-01.md`. Chapters 31–40 remain, so create **exactly one** next-phase directory at `workspace/volume-01/batch-0004/` with a detailed `PROMPT.md` for Chapters 31–40, beginning from the state this batch actually leaves behind. Do not create more than the next phase, and do not write a volume-close prompt, because the volume is not finished.
